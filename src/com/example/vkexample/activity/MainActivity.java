...
public class MainActivity extends BaseActivity {
	...
	if (requestCode == VK_AUTH_REQUEST_CODE) {
		...
		String token = data.getStringExtra(AuthActivity.OUT_EXTRA_TOKEN);
		Long userId = data.getLongExtra(AuthActivity.OUT_EXTRA_USER_ID, 0);
	...
	if (!mSession.isActive())
	    startActivityForResult(new Intent(this, AuthActivity.class), VK_AUTH_REQUEST_CODE);
	...
	case R.id.refresh_wallmessages:
	    ((VkExampleApplication) getApplication()).getVkApiHelper().wallGet(mReceiver, 0, 20,
		    mSession.getAccessToken(), mSession.getUserId());
	    break;
	default:
	    break;
	}
	...
	if (data.getString(VKRestApiHelper.EXTRA_RESULT_ACTION).equals(WallGet.class.getSimpleName())) {
